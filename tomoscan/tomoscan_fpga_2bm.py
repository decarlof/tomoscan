...
class TomoScan2BM(TomoScanFPGAPSO):
    ...
    def add_theta(self):
        """Add theta at the end of a scan (supports uniform + interlaced).

        Uses NDArrayUniqueId and HDF5FrameLocation to map recorded projection frames
        back to the intended theta list, so dropped frames are handled.
        """
        log.info("add theta")
        ...
        try:
            with h5py.File(full_file_name, "a") as f:
                if self.theta is None:
                    log.warning("add_theta: self.theta is None, skipping theta write.")
                    return

                # Intended theta list for projections
                theta_all = np.asarray(self.theta, dtype=np.float64)
                n_theta = int(theta_all.size)

                unique_ids = f["/defaults/NDArrayUniqueId"]
                hdf_location = f["/defaults/HDF5FrameLocation"]

                total_dark_fields = self.num_dark_fields * (
                    (self.dark_field_mode in ("Start", "Both")) + (self.dark_field_mode in ("End", "Both"))
                )
                total_flat_fields = self.num_flat_fields * (
                    (self.flat_field_mode in ("Start", "Both")) + (self.flat_field_mode in ("End", "Both"))
                )
                ...
                if len(proj_ids) == 0:
                    log.warning("add_theta: No projection frames found at /exchange/data")
                    return

                # Map proj unique IDs -> 0-based indices into theta_all
                # This assumes theta_all[0] corresponds to the first projection trigger
                # and the camera unique IDs increment by 1 per trigger.
                base = int(proj_ids[0])
                idx = (proj_ids.astype(np.int64) - base)

                # Keep only valid indices (if something odd happened)
                valid = (idx >= 0) & (idx < n_theta)
                if not np.all(valid):
                    bad = idx[~valid]
                    log.warning("add_theta: %d proj indices out of range (dropping). Examples=%s",
                                bad.size, bad[:10].tolist())
                    idx = idx[valid]

                theta_written = theta_all[idx]

                # Create/overwrite theta dataset
                if "/exchange/theta" in f:
                    del f["/exchange/theta"]
                theta_ds = f.create_dataset("/exchange/theta", data=theta_written)

                # ---- Missing data diagnostics ----
                if len(proj_ids) != n_theta:
                    missing = n_theta - len(proj_ids)
                    log.warning("There are %d missing data frames (expected %d, recorded %d)",
                                missing, n_theta, len(proj_ids))

                    present = set(idx.tolist())
                    missed_idx = [i for i in range(n_theta) if i not in present]
                    missed_theta = theta_all[missed_idx]
                    log.warning("Missed theta (first 50): %s", missed_theta[:50].tolist())

                if len(flat_ids) != total_flat_fields:
                    log.warning("There are %d missing flat field frames", total_flat_fields - len(flat_ids))
                if len(dark_ids) != total_dark_fields:
                    log.warning("There are %d missing dark field frames", total_dark_fields - len(dark_ids))
